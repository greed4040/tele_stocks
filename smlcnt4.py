...
def add_msg(conn, task):
    sql = '''INSERT INTO 
        messages(msg_id, msg_text, msg_peer_id, msg_state, chan_id)
        VALUES(?,?,?,?,?)'''
    cur = conn.cursor()
...
def select_message_byid(conn, msg_id, chan_id):
    cur = conn.cursor()
    cur.execute("SELECT * FROM messages where msg_id=? and chan_id=?", (msg_id, chan_id,))
...
async def main222():
    ...
    channel = await clnt.get_entity('-1001442825795')    
    ...
    for msg in messages:
        ...
        #if msg.media!=None:
        if True:
            if msg.media!=None: print("  media in msg")
            if msg.text!=None: print("#text:", msg.text[0:40])
            
            if select_message_byid(conn, str(msg.id), str(id))[0]==0:
                #if True:
                data=(str(msg.id), msg.text, str(msg.peer_id), "new")
                add_msg(conn, data)
                if msg.text!=None:
                    msg.text=msg.text.replace('@Bid_Kogan','@ron_growth')
                    msg.text=msg.text.replace('@bitkogan','@ron_growth')
                await clnt.send_message("the_schrodinger_cat", msg)
                #res = await clnt.download_media(message=msg)
                #print("#res dwn:", res)
                update_msg(conn, ("saved", msg.id))
                time.sleep(3)
            
            #print("#file:", msg.file)
            #print("#file: (id)", msg.file.id)
            #print("#file: (name)", msg.file.name)
            #print("#photo: ", msg.photo)
            #print("#photo: (id)", msg.photo.id)
            #print("#photo: (access_hash)", msg.photo.access_hash)
            #print("#photo: (fileref)", msg.photo.file_reference)
            
            #print("-"*30)
            #print("#dir file:", dir(msg.file))
            #break
...
async def main():
    ...
    #for id in [-1001442825795, -1001258957480, -1001199979298]:
    for id in [-1001442825795, -1001199979298]:
        async for msg in clnt.iter_messages(id, limit=20):
            print(id, msg.id, "#"*50)
            if True:
                if msg.media!=None: print("  media in msg")
                if msg.text!=None: print("#text:", msg.text[0:40])
                ...
                if select_message_byid(conn, str(msg.id), str(id))[0]==0:
                    #if True:
                    data=(str(msg.id), msg.text, str(msg.peer_id), "new", id)
                    add_msg(conn, data)
                    if msg.text!=None:
                        msg.text=msg.text.replace('@Bid_Kogan','Privileged Info')
                        msg.text=msg.text.replace('@bitkogan','Privileged Info')
                    #await clnt.send_message("the_schrodinger_cat", msg)
                    await clnt.send_message("-1001171032888", msg)
                    #res = await clnt.download_media(message=msg)
                    #print("#res dwn:", res)
                    update_msg(conn, ("saved", msg.id))
                    time.sleep(3)
        for i in range(0,3): print("&"*20)
        pass
    pass
#async def main_test():
